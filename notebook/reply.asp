...
function Post() {
	...
	if (document.signform.subject.value.length>50)
	{
		alert("文章主题的字数太多了\n\n请精简主题内容：）");
		document.signform.subject.focus();
		return false;
...
<BR>
<TABLE cellSpacing=0 cellPadding=0 width=760 align=center border=0 height="351">
  <% response.write("<FORM name=signform onsubmit='return Post()' action='subreply.asp?reid="&rsa("noteid")&" ' method=post>")%>
  <TBODY> 
  <TR> 
      ...
      <TABLE cellSpacing=0 cellPadding=0 width=308 align=center border=0>
        <TR> 
          <TD height=36 colspan="2"> 
            ...
            <INPUT name=subject value="Re:<%=rsa("notetitle")%>">
          </TD>
        </TR>
        <TR vAlign=top> 
          <TD height=137 colSpan=2> 
              <table cellspacing=1 cellpadding=1 width=319>
                ...
                <td class=text1 noWrap height=33> 
                  ...
                  name=face>
                  <img height=15 
                  src="img/biaoqing/1.gif" width=15>我来回答你</td>
                <td class=text1 noWrap height=33> 
                    <input type=radio value=3 
                  name=face>
                    <img height=15 src="img/biaoqing/2.gif" 
                  width=15>号外！号外！</td>
              </tr>
              <tr> 
                <td class=text1 noWrap height=33> 
    ...
    <TD vAlign=top width=408 height=324> 
      <TABLE cellSpacing=0 cellPadding=0 width=360 align=center border=0>
        ...
        <TR vAlign=top> 
          <TD colSpan=2> 
            <DIV align=left> 
              <TEXTAREA name=message rows=15 cols=54>在 <%=rsa("notewriter")%> 的大作中提到：<%=rsa("notetext")%>
			  </TEXTAREA>
            </DIV>
          </TD>
        </TR>
